# -*- coding: utf-8 -*-
"""Using_Mecab_Naver_Review_perplexity v0.2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1YHdK25mA4HMCZttLOBVWM3q0V2Yt_v9v
"""
...
data = pd.read_csv('ID-Review.csv', encoding='utf-8').drop('Unnamed: 0', axis=True)
data.head()
...
data['FReview'] = [[] for _ in range(data.shape[0])]
data.head()

import re
for j in range(len(data)) :
  for i in range(len(data.Review[j].split(','))) :
    data.FReview[j].append(re.sub('[-=+,#/\?:^$.@*\"※~&%ㆍ!』\\‘|\(\)\[\]\<\>`\'…》]','',data.Review[j].split(',')[i]))

for i in range(len(data.FReview)) :
  for k in range(len(data.FReview[i])) :
    if '' == data.FReview[i][k] :
      data.FReview[i][k] = 'None'
...
extract_corpus_mecab = extract_corpus_mecab(data.FReview[1])
