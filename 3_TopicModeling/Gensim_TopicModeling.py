# -*- coding: utf-8 -*-
"""Gensim_Topic_Modeling v0.2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ip8EVGDG59gXtldSm9wbWn-S95wH4I96
"""
...
#n_tags_all = ['NNG', 'NNP', 'NNB', 'XR', 'VV', "VA"]
'''
...
bigram = gensim.models.Phrases(khaiii_all, min_count=5, threshold=100) # higher threshold fewer phrases.
trigram = gensim.models.Phrases(bigram[khaiii_all], threshold=100)
...
khaiii_all = remove_stopwords(khaiii_all)

khaiii_all = make_trigrams(khaiii_all)
...
for i in range(len(khaiii_all)):
  for j in range(len(khaiii_all[i])):
    khaiii_all[i][j] = khaiii_all[i][j].replace("_", "")
...
cv = vectorizer.fit_transform(detoken_all)
...
coherence_model_lda = CoherenceModel(model=model, texts=khaiii_all, dictionary=dic, coherence='c_v')
...
def compete_number_of_words(detoken_data, token_data, min_num, max_num, step, random_state=None):
  ...
  for feature in range(min_num, max_num, step):
    ...
    cv = vectorizer.fit_transform(detoken_all) # fit and transform
...
compete_num_words = compete_number_of_words(detoken_all, khaiii_all, 1000, 32000, 1000, random_state=42)
...
best_count = CountVectorizer(max_features=12000)

count_vec =  best_count.fit_transform(detoken_all)
...
for n_topic in topic_range:
  for a in alpha:
    for b in beta:
      ...
      cv = compete_coherence_values(corpus=corpus, token_data=khaiii_all, id2word=dic, k=n_topic, a=a, b=b)
